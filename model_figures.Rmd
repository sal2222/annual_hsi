...
plot_day_indices <-
  ...
  filter(!index %in% c("days_wbgt_max_gt1sd",
                       ...
                       "days_tmp_max_gt2sd",
                       "days_hi_gt80",
                       "days_tmp_max_gt90pct")) %>%     # filter selection of "null" indices, tight CIs at RR 1.0
  ggplot() +
        geom_point(aes(x = exp(statistic), y = index), shape = 1, fill = "white", color = "black", size = 1) +
        geom_point(aes(x = exp(statistic - bias), y = index), size = 1) +
        geom_errorbarh(
...
ggsave(filename = "2yr_day_hour_split_plot.png", path = "output", width = 7, height = 8)
