...
amb_boot_2yr_block_hs <- read_rds("data/amb_boot_2yr_block_hs.rds")
hosp_boot_2yr_block_hs <- read_rds("data/hosp_boot_2yr_block_hs.rds")
rme_boot_2yr_block_hs <- read_rds("data/rme_boot_2yr_block_hs.rds")
...
#  ggtitle("Combined HSI outcome rates for ten CONUS Army installations") +
 ...
 # labs(caption = "The line represents a linear model and the shaded area models 95% confidence levels. Note that the scales vary by outcome category\nby orders of magnitude.") +
  theme_bw() +
...
ggsave(filename = "figure1.png", path = "output", width = 6, height = 3)
...
to_fig2 <-
  ...
  ) %>% filter(index %in% c("tmp_f_mean", "heat_index_mean", "wbgt_f_mean")) %>% 
    mutate(avg_period = "Full year") %>% 
    bind_rows(
  amb_boot_2yr_block_hs %>% 
    unnest(tidy) %>%
    mutate(type = "AMBULATORY",
           avg_period = "May-Sep") %>% 
    dplyr::select(-amb_data)  
  ) %>% bind_rows(
    hosp_boot_2yr_block_hs %>% 
    unnest(tidy) %>%
    mutate(type = "HOSPITALIZATIONS",
           avg_period = "May-Sep") %>% 
    dplyr::select(-hosp_data)
  ) %>% bind_rows(
  rme_boot_2yr_block_hs %>% 
    unnest(tidy) %>%
    mutate(type = "REPORTABLE EVENTS",
           avg_period = "May-Sep") %>% 
    dplyr::select(-re_data)
    ...
    mutate(index = fct_relevel(index, c("wbgt_f_mean", "heat_index_mean")),  # orders from bottom to top
...
ggsave(filename = "figure2.png", path = "output", width = 8, height = 5)
